...
document.addEventListener('DOMContentLoaded', function() {
	...
	cancelRequestButtons.forEach(button => {
		button.addEventListener('click', function(event) {
			...
			fetch(`${window.location.origin}/cancel-request/${userId}/`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'X-CSRFToken': '{{ csrf_token }}'
			...
			.then(data => {
				if (data.success) {
					buttonElement.parentElement.innerHTML = '<span class="badge badge-danger text-bg-danger">Rejected</span>';
					// 
				} else {
					alert('Failed to cencel friend request.');
				}
			})
			.catch(error => {
				console.error('Error:', error);
